...
pio run -e <env> -t uploadfs              # flash data/ (LittleFS) - config + web UI. Auto-rebuilds web/ first if
                                           #   node+npm are installed, else warns + flashes committed data/www (Gotcha #13)
...
tools/                   # build_web.py (buildfs/uploadfs web-UI rebuild hook, Gotcha #13)
                         #   + HIL harness (verify.sh / boot_check.py / hil_smoke.py)
...
- REST surface (full spec in README, `src/net/webApi.{h,cpp}`): `GET/POST /config`, `GET /status` (lightweight poll endpoint — the runtime `info` fields plus `_needReboot`, via `Config::needsReboot()`; the web UI polls it every 5 s to clear its reboot banner once the device reboots), `POST /reboot`, `POST /reset-wifi`, `GET /heap`. The `info` block (in both `/config` and `/status`) also carries an `ota` bool — `false` when `DISABLE_OTA` strips ElegantOTA's `/update` (sonoff envs) — which the web UI's System tab uses to show/hide its "Open firmware updater" link. `POST /config` partial updates are supported, but `dmx[]` is index-based so **all** elements must be sent together when updating that array; `_needReboot` (`Config::_dirty`, reset on boot, not persisted) flags pending changes.
...
- **Canonical contract + standing rule (READ BEFORE TOUCHING THE REST API OR WEB UI):** [`docs/API_UX_DESIGN.md`](docs/API_UX_DESIGN.md) is the **authoritative, versioned** spec of the REST API and the configuration SPA's UX (it wins over README/code comments on conflict). Keep the REST API **backwards-compatible on a best-effort basis** — additive only: don't remove/rename/retype an existing field, endpoint, or status code; new fields are optional with safe defaults; keep accepting legacy values (e.g. `BINARY`/`RELAY`, `dhcp` always-true); a genuine breaking change needs a `CONFIG_SCHEMA_VERSION` bump + migration on the `configVersion` hook. **Any** change to an endpoint, the config schema, the `info` fields, the auth model, or the SPA's information architecture / per-section fields / save semantics / visibility rules **MUST**, in the same change, update that doc, bump its semver **contract version**, and add a changelog entry (`Compatible`/`Breaking`) — that's how other agents catch up from any point.
...
- **[`docs/API_UX_DESIGN.md`](docs/API_UX_DESIGN.md)** — the **canonical, versioned** REST API + config-SPA UX contract for client/UI authors (see the §7 standing rule). Authoritative on conflict; keep it updated + version-bumped on every REST/UX change.
